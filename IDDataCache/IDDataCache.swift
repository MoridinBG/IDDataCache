...
public class IDDataCache
{
	...
	*/
	static public var sharedPersistentInstanceNames: [String]
	{
		get
		{
			return [String](sharedNamedPersistentInstances.keys)
		}
	}
	...
	*/
	...
	public init(namespace: String, isPersistent: Bool)
	{
		...
		print("Caching at \(self.diskCachePath)")
	...
	/**
	* Add a read-only cache path to search for data pre-cached by IDDataCache
	...
	*/
	public func cachePathForKey(key: String, path: NSString) -> String
	{
	...
	*/
	...
	/**
	* Store a data object into memory and optionally disk cache for the given key.
	*
	...
	*/
	...
	/**
	* Store a data object into memory and disk cache for the given key.
	*
	* @param data  The data to store
	* @param key   The unique data cache key, usually it's data absolute URL
	*/
	...
	*/
	public func queryDiskCacheForKey(key: String, done: (data: NSData, cacheType: IDDataCacheType) -> Void) -> NSOperation?
	{
		...
		dispatch_async(ioQueue) {
			...
			if let diskData = self.diskDataForKey(key)
			{
	...
	*/
	public func dataFromDiskCacheForKey(key: String) -> NSData?
	{
		...
		if let diskData = diskDataForKey(key)
		{
	...
	*/
	public func cleanDiskWithCompletionBlock(completion: (() -> Void)?)
	{
		dispatch_async(ioQueue) {
			...
			let fileEnumerator = self.fileManager.enumeratorAtURL(diskCacheURL, includingPropertiesForKeys: resourceKeys, options: [], errorHandler: nil)!
			...
			for element in fileEnumerator
			{
				...
				do
				{
					...
					// Skip directories.
					// TODO: May fail
					if let isDirectory = resourceValues[NSURLIsDirectoryKey] as? Bool where isDirectory == true
					{
						continue
					}
				...
				{
					log.error("Couldn't get resources for path \(fileURL.absoluteString)")
				}
			}
	...
	func getSize() -> Int
	{
		...
		dispatch_sync(ioQueue) {
			...
			for fn in fileEnumerator
			{
				...
				let filePath = (self.diskCachePath as NSString).stringByAppendingPathComponent(filename)
	...
	*/
	...
	private func diskDataBySearchingAllPathsForKey(key: String) -> NSData?
	{
	...
	private func diskDataForKey(key: String) -> NSData?
	{
		if let data = diskDataBySearchingAllPathsForKey(key)
		{
			return data
		}
